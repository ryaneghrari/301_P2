...
// Initializes all the fields for every instruction in Opcode enum
{
  ...
  myArray[SRA].name = "srl";  myArray[SRA].numOps = 3;   myArray[SRA].rdPos = 0;  myArray[SRA].rsPos = -1;  myArray[SRA].rtPos = 1;   myArray[SRA].immPos = 2;  myArray[SRA].instType = RTYPE; myArray[SRA].op_field = "000000"; myArray[SRA].funct_field = "000011";
  ...
  myArray[J].name = "j";   myArray[J].immLabel = true;  myArray[J].numOps = 1;   myArray[J].rdPos = -1;  myArray[J].rsPos = -1;  myArray[J].rtPos = -1;   myArray[J].immPos = 0;  myArray[J].instType = JTYPE; myArray[J].op_field = "000010"; myArray[J].funct_field = "000000";

  myArray[BEQ].name = "beq";  myArray[BEQ].immLabel = true; myArray[BEQ].numOps = 3;   myArray[BEQ].rdPos = -1;  myArray[BEQ].rsPos = 0;  myArray[BEQ].rtPos = 1;   myArray[BEQ].immPos = -1;  myArray[BEQ].instType = ITYPE; myArray[BEQ].op_field = "000100"; myArray[BEQ].funct_field = "000000";
}
...
string OpcodeTable::getOp(Opcode o)
// Given a valid MIPS assembly mnemonic, returns an Opcode which represents a 
// template for that instruction.
{
  if(o < 0 || o >= UNDEFINED)
...
Opcode OpcodeTable::findOpcode(string str)
// Given a valid binary string returns an Opcode which represents a 
// template for that instruction.
{
  for(int i = 0; i < (int)UNDEFINED; i++){
...
Opcode OpcodeTable::findRtypeOpcode(string funct)
{
  for(int i = 0; i < (int)UNDEFINED; i++){
